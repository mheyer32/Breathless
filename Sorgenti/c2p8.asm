...
;*	c2p8.asm
;*
;*	Routine di conversione chunky to planar
;*
;*
;* - Nuova gestione della conversione per schermi con pixel 1x2 o 2x2
;*   Ora scrive nello schermo destinazione un rigo si e uno no.
;*
;****************************************************************************
...
;
; Modified by Alberto Longo, 26 Aug 1994
;
;
...
;	Max framerate is 62ms/frame = 16fps
;	 occurs when fBUFFER rendering time <= 21ms (= 62ms-41ms)
;
;
;
; -------------------------------------------------------------------
...
c2p8_init:
		...
		move.l	a1,(mode-mybltnode,a4)
		move.l	d0,(signals1-mybltnode,a4)
		move.l	d1,(signals2-mybltnode,a4)
		move.l	d5,(tmp_buff2-mybltnode,a4)
		move.l	d6,(tmp_buff3-mybltnode,a4)
		...
		bsr	c2p8_waitblitter

		move.l	a1,d0
		btst	#0,d0	;Test conversion mode
		beq.s	c2p_nodoublex
		lsl.l	#1,d2	;window width*=2
c2p_nodoublex
		move.l	d2,d1
		mulu	d3,d1	;d1=# pixels

		move.l	d1,(pixels-mybltnode,a4)
		lsr.l	#1,d1
		move.l	d1,(pixels2-mybltnode,a4)
		lsr.l	#1,d1
		move.l	d1,(pixels4-mybltnode,a4)
		lsr.l	#1,d1
		move.l	d1,(pixels8-mybltnode,a4)
		lsr.l	#1,d1
		move.l	d1,(pixels16-mybltnode,a4)

		move.l	d7,d1
		lsr.l	#3,d1
		sub.l	d2,d7	;d7=screen_width - window_width
		lsr.l	#3,d7
		move.l	d7,(mod-mybltnode,a4)
		lsr.l	#1,d7
		add.l	d7,d4	;offset+=(screen_width - window_width)/16
		move.l	#200,d7
		btst	#1,d0
		beq.s	c2p_nodoubley
		lsr.l	#1,d7
		add.w	d1,(mod+2-mybltnode,a4)
		add.l	d1,d1
c2p_nodoubley
		sub.l	d3,d7
		lsr.l	#1,d7	;d7=(screen_height - window_height)>>1
		mulu	d1,d7
		add.l	d7,d4
		move.l	d4,(offset-mybltnode,a4)

		lsr.l	#4,d2
		move.l	d2,(width-mybltnode,a4)
		move.l	d3,(height-mybltnode,a4)

		subq.l	#1,d3
		mulu	(mod+2-mybltnode,a4),d3		;d3=mod*(window_height-1)
		add.l	(offset-mybltnode,a4),d3
		move.l	d3,(offset2-mybltnode,a4)
...
;************************************************************************
;* Attende che il blitter abbia finito

		xdef	c2p8_waitblitter

c2p8_waitblitter
		movem.l	d0-d7/a0-a6,-(sp)

		lea	mybltnode(pc),a2	; a2->mybltnode
		move.l	(4).w,a6

		move.l	(signals1-mybltnode,a2),d0
		jsr	(_LVOWait,a6)		; Test signal1

		move.l	(signals2-mybltnode,a2),d0
		jsr	(_LVOWait,a6)		; Test signal2

		move.l	(signals1-mybltnode,a2),d0
		move.l	d0,d1
		jsr	(_LVOSetSignal,a6)	; Set signal1

		move.l	(signals2-mybltnode,a2),d0
		move.l	d0,d1
		jsr	(_LVOSetSignal,a6)	; Set signal2
...
c2p8_go:	movem.l	d2-d7/a2-a6,-(sp)

		lea	mybltnode(pc),a2

		move.l	a0,-(sp)	;Salva puntatore ai planes

; wait for previous call to c2p8 to finish pass 3
...
;-------------------------------------------------
		...
		btst	#0,(mode-mybltnode+3,a0)	;test conversion mode
		bne	c2p_doublew
		...
		move.l	a1,a3		; a3 = for finish test

		move.l	#$0f0f0f0f,d7	; constant
		move.l	#$00ff00ff,d6	; constant
...
;First 16 pixels
		move.l	(a2)+,d0	; AaBbCcDd
		move.l	(a2)+,d1	; EeFfGgHh
		move.l	(a2)+,d2	; IiJjKkLl
		move.l	(a2)+,d3	; MmNnOoPp
...
go_end1		cmpa.l	a3,a0		;Test finish
		blt	c2p_loop
...
c2p_doublew
		move.l	(pixels8-mybltnode,a0),d6	; loop count
		lsr.l	#1,d6
...
; convert 8 pixels (passes 1 and 2 combined) into 16 pixels

c2p_loop2
		move.l	(a2)+,d0	;d0=AaBbCcDd
		move.l	(a2)+,d1	;d1=EeFfGgHh

		ror.w	#4,d0		;d0=AaBbdCcD
		move.l	d0,d2
		and.l	d5,d0		;d0=A.B..C.D
		eor.l	d0,d2		;d2=.a.bd.c.

		ror.w	#4,d1		;d1=EeFfhGgH
		move.l	d1,d3
		and.l	d5,d1		;d1=E.F..G.H
		eor.l	d1,d3		;d3=.e.fh.g.


		move.l	d0,d4
		swap	d0		;d0=.C.DA.B.
		or.l	d4,d0		;d0=ACBDACBD
		clr.b	d0		;d0=ACBDAC..
		swap	d0		;d0=AC..ACBD
		lsl.w	#8,d0		;d0=AC..BD..
		...
		move.l	d1,d4
		swap	d1		;d1=.G.HE.F.
		or.l	d4,d1		;d1=EGFHEGFH
		lsr.l	#8,d1		;d1=..EGFHEG
		lsr.w	#8,d1		;d1=..EG..FH

		or.l	d1,d0		;d0=ACEGBDFH
		move.l	d0,(a0)+

		lsl.l	#4,d2		;d2=a.bd.c..
		move.l	d2,d4
		swap	d2		;d2=.c..a.bd
		or.l	d4,d2		;d2=acbdacbd
		clr.b	d2		;d2=acbdac..
		swap	d2		;d2=ac..acbd
		lsl.w	#8,d2		;d2=ac..bd..
		...
		lsl.l	#4,d3		;d3=e.fh.g..
		move.l	d3,d4
		swap	d3		;d3=.g..e.fh
		or.l	d4,d3		;d3=egfhegfh
		lsr.l	#8,d3		;d3=..egfheg
		lsr.w	#8,d3		;d3=..eg..fh

		or.l	d3,d2		;d2=acegbdfh

		move.l	d2,(a4)+
		move.l	d2,(a5)+

go_end2		dbra	d6,c2p_loop2
...
blit47:		movem.l	a2,-(sp)
		...
		move.w	#0,(bltamod,a0)
		move.w	#0,(bltbmod,a0)
		move.w	(mod+2-mybltnode,a1),(bltdmod,a0)
		move.w	(height+2-mybltnode,a1),(bltsizv,a0)	; height
		move.w	#$aaaa,(bltcdat,a0)
		move.l	#$0DE41000,(bltcon0,a0)	; D=AC+(B>>1)~C
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 7
...
blit43:		move.l	a2,d1			; preserve a2
		...
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 3
		lea	(blit45,pc),a0
		move.l	a0,(qblitfunc-mybltnode,a1)
...
blit45:		move.l	a2,d1			; preserve a2
		...
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 5
		lea	(blit41,pc),a0
		move.l	a0,(qblitfunc-mybltnode,a1)
		move.l	d1,a2			; restore a2
		rts
...
blit41:		move.l	a2,d1			; preserve a2
		...
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 1
		lea	(blit46,pc),a0
		move.l	a0,(qblitfunc-mybltnode,a1)
		move.l	d1,a2			; restore a2
		rts
...
blit42:		move.l	a2,d1			; preserve a2
		...
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 2
		lea	(blit44,pc),a0
		move.l	a0,(qblitfunc-mybltnode,a1)
		move.l	d1,a2			; restore a2
		rts
...
blit44:		move.l	a2,d1			; preserve a2
		...
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 4
		lea	(blit40,pc),a0
		move.l	a0,(qblitfunc-mybltnode,a1)
		move.l	d1,a2			; restore a2
		rts
...
blit40:		move.l	a2,d1			; preserve a2
		...
		move.w	(width+2-mybltnode,a1),(bltsizh,a0)	;plane 0
		lea	(blit31,pc),a0
		move.l	a0,(qblitfunc-mybltnode,a1)
		move.l	d1,a2			; restore a2
...
offset2:	dc.l	0		; byte offset into plane for descending blitting
