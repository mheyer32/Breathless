...
Tloop
		tst.l	terminal(a5)		;Il terminale è ancora attivo ?
		...
		beq	Tloop			;Se sono uguali, allora la coda è vuota
...
DSbsnoexit	move.l	CurrentPage(a5),a0
		...
		moveq	#0,d1
		move.w	8(a0),d1		;d1=credits necessari
		bmi.s	DSnobuy			; Se<0, non si può acquistare
		move.l	PlayerCredits(a5),d2	;d2=credits posseduti
		sub.l	d1,d2			;Sottrae crediti
...
DSacnoexit	move.l	CurrentPage(a5),a0
		...
		blt.s	DSacj1
		addq.w	#1,d4
DSacj1		move.w	10(a0),d2		;Qtà da sommare
		jsr	CollectItem
...
DSCMkeloop	jsr	TestChangeScreen
		...
		tst.l	(a0)			;Test se il tasto è utilizzabile
...
;*	numpun	 L	presente solo se type=1, è il pun. a una long
;*			contenente il numero da stampare
;*	credits  W	presente solo se type=-1, è il num. di crediti
;*			necessario per acquistare l'oggetto
;*	value	 W	presente solo se type=-1, è la quantità
;*			che si acquista
;*	cyclex	 W	presente solo se type=2 o 3 o 4, è la coordinata x della stringa di selezione (o di input)
;*	cycley	 W	presente solo se type=2 o 3 o 4, è la coordinata y della stringa di selezione (o di input)
;*	cyclepun L	presente solo se type=2, è il pun. alla struttura che
;*			contiene i pun. alle stringhe selezionabili
;*	cyclesel L	presente solo se type=2, è il pun. alla word
;*			che contiene l'indice, all'interno della struttura
;*			puntata da cyclepun, della stringa selezionata
;*	keypun	 L	presente solo se type=3, è il pun. alla lista dei nomi
;*			dei tasti della tastiera, ordinati per scancode.
;*			Ogni nome è una stringa di 3 char, terminata con uno zero.
;*	keysel	 L	presente solo se type=3, è il pun. alla word
;*			che contiene lo scancode del tasto selezionato
;*	destpun  L	presente solo se type=4, è il pun. alla stringa null term. che contiene il
;*			valore iniziale della stringa di input e che, finito l'input,
;*			contiene i caratteri immessi dall'utente
;*	inputpun L	presente solo se type=4, è il pun. al buffer di input
...
terminal	ds.l	1	;Se<>0, contiene il num. del terminale attivo
				;Se<0, è il menu di configurazione
...
Protection	ds.b	1	;Se=TRUE, è attivo l'input del codice di protezione
